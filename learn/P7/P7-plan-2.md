...
## 2026-04-24 状态更新（F1/F2 验证结果 + Cursor 对齐）

### Chrome 143.0.7499.169 第二次长跑结果：F1 验证**完全通过**

对比 `results/chrome_143.0.7499.169_linux_x86_64.json`（auto）与 `chrome_143.0.7499.169_linux_x86_64.json`（ground truth）：

| Hook | RVA 匹配 | Fingerprint 逐字节匹配 | fingerprint_len |
|---|---|---|---|
| hkdf | ✅ `0x048837E0` | ✅ 106B 完全一致 | 106 |
| ssl_log_secret | ✅ `0x04883520` | ✅ 34B 完全一致 | 34 |
| prf | ✅ `0x0A22D4B0` | ✅ 40B 完全一致 | 40 |
| key_expansion | ✅ `0x0A22D130` | ✅ 81B 完全一致 | 81 |

**结论**：TSHunter 自动分析器在 "**找 hook 点 + 抽字节指纹**" 这件事上已经**生产级可用**。阶段 F1 不需要再投 17h 验证。

### 自动结果 vs ground truth 的差距（"周边字段"差距，**不是定位差距**）

差距可归为三类，**每类都对应 Cursor P7-1 的建议落点**：

| 差距类别 | 示例字段 | Cursor 建议的归属 | 实现位置（我的 plan 对应） |
|---|---|---|---|
| **结构层** —— 命名/格式不统一 | `function` vs `function_name`+`ghidra_name`；`image_base: null` vs `ghidra_image_base`；大小写 `boringssl` vs `BoringSSL` | 统一 JSON schema | Phase **S2**（新增） |
| **语义层** —— Frida 注入必需参数 | `params`（RDI/RSI...）；`read_on: onLeave`；`output_len`；`client_random path` | 运行时模板层（profiles/） | Phase **U2** 扩展（新增 `runtime_templates`） |
| **验证层** —— 实测指标 | `verified: true`；`verified_method`；`p3_capture_rate`；`p4_tuple_hit_rate`；`boringssl_commit` | 独立后处理脚本 | Phase **S2b** + `tools/annotate_verified.py` |

### Cursor 的三个关键洞察（已吸收进本 plan）

1. **"先消费端，后生产端"**：在动 TSHunter schema 之前，先审计 p_t_c 运行时到底需要哪些字段 → **新增 Phase S1**。
2. **"分析 / 验证 / 运行时模板三层分离"**：
   - 分析层（TSHunter run.py 输出）—— 已 OK，RVA + fingerprint
   - 验证层（独立工具注入实测数据）—— 后处理脚本，**新增**
   - 运行时模板层（按 TLS 栈跨版本复用）—— `profiles/boringssl_chrome.json` 等，**新增**
3. **"不要把所有字段塞进 run.py"**：`client_random path`、`tls13_label_map`、`struct_offsets` 这些跨版本稳定的知识应该抽到模板层，单 version 的 hook_points 只放 `rva`/`fingerprint`/`role`/`params` 这类版本相关信息。

### 对原 plan 的增量（4 个要点）

1. **F1/F2 已完成，从执行列表移除**。保留"验证通过记录"作追溯。
2. **U0 之前插入 S1（p_t_c 审计）和 S2（schema 统一）两阶段**。这两阶段不涉及任何代码大改，只产出**设计文档** + **TSHunter `run.py` 输出的结构性微调**，是所有后续批量/整合的前置。
3. **U2 阶段扩展为三层**：DB 作 SoT（已有）+ `profiles/` 模板层（新增）+ `tools/annotate_verified.py` 验证层（新增）。`VersionConfigLoader` 同时 merge 三层后再返回给 `capture.py`。
4. **B1 批量阶段的入库路径**：每一个版本入库时自动带 `profile_ref`（指向 profiles/ 的模板名）+ `verified=false`（除非 `annotate_verified.py` 显式打过）。
